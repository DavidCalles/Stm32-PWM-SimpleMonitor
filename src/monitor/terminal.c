...
#endif
#if (defined(STM32F303xE) || defined(STM32G474xx) || defined(STM32L432xx))
    uart->RQR = USART_RQR_TXFRQ; /* Clear transmit done */
#elif (defined(STM32F411xE))
    /* NOP */
#else
#error "No definition of RQR register in USART"
#endif
  }
}
...
#elif (defined(STM32F411xE))
  if(uart->SR & USART_ISR_RXNE) {
    *ptr = uart->RDR;
    return 1;
  }
#else
#error "No definition of Status register in USART"
#endif
...
uint32_t TerminalWritePolled(PortIndex_e index,const char *p, uint32_t len)
{
  ...
  while(len) {
    ...
    uart->DR = *p;
#else
#error "No definition for Status register in USART"
#endif
...
void TerminalISR(PortIndex_e index)
{
  ...
  uint32_t icr = 0;
#elif (defined(STM32F411xE))
  /* No ICR in this processor. */
#else
#error "No definition for Interrupt clear register in USART"
#endif
...
#error "No definition for Status register in USART"
  ...
  uart->ICR = icr;
#elif (defined(STM32F411xE))
  /* NOP - errors clear themselves in this chip. */
#else
#error "No definition for Status register in USART"
#endif
